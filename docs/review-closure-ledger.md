...
| Hosted exact must not advertise before Worker settlement exists | Fixed for current hosted gateway | `demo.kaspa-x402.org` keeps `exact` unadvertised until the hosted verifier/broadcast/observe path is implemented and tested. Inventory registration is admin-only staging, expired reservation leases retire instead of becoming reusable, and batch inventory registration is atomic. |
