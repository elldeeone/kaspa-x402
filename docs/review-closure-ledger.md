...
| Hosted exact must not advertise before Worker settlement exists | Fixed for current hosted gateway | `demo.kaspa-x402.org` advertises `exact` only when hosted exact settlement is enabled and funded inventory is available. Inventory registration is admin-only, exact transaction artifacts are verified and matched to accepted chain evidence before content is served, expired reservation leases retire instead of becoming reusable, and batch inventory registration is atomic. Current public REST submit does not preserve tx-v1 compute budget, so hosted exact proof pre-broadcasts through a Kaspa RPC path before Worker observation. |
