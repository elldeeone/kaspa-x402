...
- **`exact`**: a fixed-price one-shot native KAS transfer. Alpha.8 makes ordinary `standard-native` transfer the default. Optional KIP-10 `additive` consumes and recreates a durable merchant head; the successor increase equals the advertised exact amount and is the only merchant payment. Unpaid 402s do not consume or retire heads. KIP-9 storage mass depends on the complete transaction shape and does not define a universal consensus dust constant.
