...
- **Live gateway**: https://demo.kaspa-x402.org is a public testnet-10 server for batch-settlement and KIP-10 exact-transaction integration. Hosted exact is advertised only while funded borrow-UTXO inventory is available. The Worker runs an automated public health check and holds no spending keys.
...
- **`exact`**: a fixed-price one-shot native KAS transfer. Alpha.6 uses the KIP-10 path: the server advertises buildable reservation terms and the client returns a signed `exact-transaction` artifact for server/facilitator verification, broadcast if needed, and observation. The reservation inventory is intended to be merchant-owned, with a minimum additive top-up so borrow UTXOs cannot be churned cheaply. KIP-9 storage mass depends on the complete transaction shape; `10000000` sompi is the reference implementation's conservative output policy, not a universal consensus dust constant.
