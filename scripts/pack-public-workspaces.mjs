...
const args = [
  ...
  "--dry-run",
  "--json",
  ...PUBLIC_PACKAGES.flatMap((name) => ["--workspace", name]),
];
...
const packed = normalizeNpmPackOutput(output);
const names = packed.map((entry) => entry?.name).sort();
const expected = [...PUBLIC_PACKAGES].sort();
if (JSON.stringify(names) !== JSON.stringify(expected)) {
  throw new Error(
    `unexpected packed workspaces: expected ${expected.join(", ")}, got ${names.join(", ")}`,
  );
}
