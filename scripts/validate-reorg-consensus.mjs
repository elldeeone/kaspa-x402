...
if (run("git", ["-C", kaspa, "rev-parse", "HEAD"]) !== expectedCommit)
  throw new Error(`Expected canonical consensus commit ${expectedCommit}`);
if (
  fs.realpathSync(run("git", ["-C", kaspa, "rev-parse", "--show-toplevel"])) !==
  kaspa
)
  throw new Error("Pass the Git checkout root.");
if (
  run("git", ["-C", kaspa, "status", "--porcelain", "--untracked-files=all"])
)
  throw new Error("Canonical checkout must be clean, including untracked files.");
if (
  run("git", ["-C", kaspa, "ls-files", "-v"])
    .split("\n")
...
const temporary = fs.mkdtempSync(
  path.join(os.tmpdir(), "x402-reorg-consensus-"),
);
try {
  fs.mkdirSync(path.join(temporary, "src"));
  fs.copyFileSync(
    path.join(root, "tools/reorg-consensus/src/main.rs"),
    path.join(temporary, "src/main.rs"),
  );
  const dependencies = {
    "kaspa-consensus": "consensus",
    "kaspa-consensus-core": "consensus/core",
    "kaspa-addresses": "crypto/addresses",
    "kaspa-hashes": "crypto/hashes",
    "kaspa-txscript": "crypto/txscript",
  };
  fs.writeFileSync(
...
${Object.entries(dependencies)
  .map(
    ([name, subdirectory]) =>
      `${name} = { path = ${JSON.stringify(path.join(kaspa, subdirectory))} }`,
  )
  .join("\n")}
`,
  );
