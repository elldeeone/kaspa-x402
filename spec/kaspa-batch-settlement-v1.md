...
Use [exact](kaspa-exact-v1.md) for fixed-price one-shot purchases. Use [upto](kaspa-upto-v1.md) for one-shot variable purchases where value should move immediately after the request.
...
| `extra.serverPublicKey` | yes | Server key allowed to verify, claim, and refund according to the covenant rules. |
...
```json
{
  ...
  "voucher": {
    "amount": "2500000",
    "signature": "<64-byte Schnorr zero-charge voucher signature hex>"
  }
}
```
...
For refund payloads, the voucher must be zero-charge: `voucher.amount == signedMaxClaimable`. The protected handler is not invoked.
