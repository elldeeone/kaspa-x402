...
The active batch covenant uses signed 64-bit script arithmetic. Every batch
monetary or covenant-state value is therefore further capped at
`9223372036854775807`; the exact profiles retain the common unsigned range.
...
- KIP-20 covenant IDs provide stable identity and authorized lineage, while
  the script still enforces every state transition;
- batch runtimes must persist the current outpoint because a covenant ID does
  not provide standard reverse lookup of its live UTXO;
...
- preserve exact current-head identity and verify same-covenant lineage through
  batch genesis, claim, top-up, and refund flows.
